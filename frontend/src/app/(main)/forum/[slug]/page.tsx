  ...
  import PostUserForum from "./posts/page";
  ...
  const ForumRecommendation = ({
    ...
    const [selectedAnswers, setSelectedAnswers] = useState<{ [key: string]: number }>({}); // Lưu lựa chọn theo topicId: index
    ...
    // Hàm xử lý rời nhóm (placeholder - bạn có thể connect API)
    const handleLeaveGroup = () => {
      if (confirm('Bạn có chắc chắn muốn rời nhóm này?')) {
        // TODO: Gọi API rời nhóm
        console.log('Rời nhóm:', slug);
      }
    };

    // Hàm xử lý xem posts user (placeholder - redirect hoặc fetch)
    const handleViewUserPosts = () => {
      // TODO: Redirect đến trang posts user hoặc fetch data
      console.log('Xem posts user');
    };

    if (!forum) return <p className="text-center mt-5">Đang tải...</p>;

    const groupedTopics = forum.forum.topics.reduce((acc: { [key: number]: TopicProps[] }, topic) => {
                      ...
                      {uniqueWeeks.map((week) => (
                        <div key={week} className="col-12">
                          <div className="d-flex align-items-center mb-4 pb-2 border-bottom border-primary border-2">
                            <h3 className="fw-bold mb-0 text-primary">Tuần {week}: {forum.forum.name}</h3>
                          ...
                          <div className="row g-4 mb-5">
                            {groupedTopics[week].map((topic) => {
                              ...
                              const questionnaire = isQuiz ? topic.questionnaires?.[0] : null;
                              const selectedIndex = selectedAnswers[topic.id];
                              const correctIndex = questionnaire ? questionnaire.options.findIndex(opt => opt === questionnaire.answer) : -1;
                              const isCorrect = selectedIndex !== undefined && selectedIndex === correctIndex;
                              const hasFeedback = selectedIndex !== undefined && questionnaire && correctIndex !== -1;
                              ...
                              return (
                                <div key={topic.id} className="col-12">
                                  <article className="card h-100 shadow-sm border-0 overflow-hidden position-relative cursor-pointer group">
                                    <div className="position-absolute top-0 end-0 w-50 h-50 bg-primary bg-opacity-10 rounded-circle translate-middle-y-n50 translate-middle-x-n50 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                    ...
                                    <div className="card-body p-4">
                                      ...
                                      {/* Content */}
                                      <h2 className="card-title fw-bold mb-3 fs-4 text-dark group-hover:text-primary transition-colors">
                                        {topic.title}
                                      </h2>
                                      
                                      {/* Description or Quiz Options */}
                                      {isQuiz && questionnaire && questionnaire.options && questionnaire.options.length > 0 ? (
                                        <div className="card-text text-secondary mb-4 lh-lg">
                                          ...
                                          <div className="mt-3 p-3 bg-white rounded border">
                                                ...
                                                return (
                                                  <li 
                                                    ...
                                                    className={`mb-2 p-2 border rounded cursor-pointer transition-colors ${
                                                      isSelected ? (isCorrect ? 'bg-success bg-opacity-10 border-success' : 'bg-danger bg-opacity-10 border-danger') : 
                                                      (hasFeedback && isCorrectOption ? 'bg-success bg-opacity-10 border-success' : 'hover:bg-light')
                                                    }`}
                                                    onClick={() => handleOptionSelect(topic.id, index)}
                                                  >
                                                    <strong className={`text-dark ${isSelected ? (isCorrect ? 'text-success' : 'text-danger') : ''}`}>
                                                      {label}.
                                                    </strong> {option}
                                                    {hasFeedback && isCorrectOption && !isSelected && (
                                                      <small className="text-success ms-2">(Đáp án đúng)</small>
                                                    )}
                                                  </li>
                                                );
                                              })}
                                            ...
                                            {hasFeedback && (
                                              <div className={`mt-3 p-2 rounded ${isCorrect ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger'}`}>
                                                <strong>{isCorrect ? '🎉 Đúng!' : '❌ Sai!'} </strong>
                                                {isCorrect ? 'Tuyệt vời!' : `Đáp án đúng là: ${String.fromCharCode(65 + correctIndex)}. ${questionnaire.answer}`}
                                              </div>
                                            )}
                                            <small className="text-muted mt-2 d-block">Chọn đáp án và thảo luận bên dưới nhé!</small>
                                          </div>
                                        </div>
                                      ) : (
                                        <p className="card-text text-secondary mb-4 lh-lg" style={{ overflow: "hidden", display: "-webkit-box", WebkitLineClamp: "3", WebkitBoxOrient: "vertical" }}>
                                      ...
                                      <div className="d-flex justify-content-between align-items-center pt-3 border-top border-light">
                                        <div className="d-flex align-items-center text-muted small">
                                          <button 
                                            ...
                                            onClick={(e) => { e.stopPropagation(); toggleForm(topic.id); }}
                                          >
                                            <i className="bi bi-chat-square-text me-1"></i>
                                            {isFormExpanded ? 'Ẩn form' : '0 Bình luận'}
                                          </button>
                                          <button className="btn btn-link p-0 text-muted hover-primary transition-colors ms-3">
                                          ...
                                          <div className="row g-3 align-items-end">
                                            ...
                                            <div className="col-12">
                                              <div className="d-flex justify-content-end">
                                                <button className="btn btn-outline-secondary btn-sm me-2 rounded-pill px-4">Hủy</button>
                                                <button className="btn btn-primary fw-semibold rounded-pill px-4 shadow-sm">
