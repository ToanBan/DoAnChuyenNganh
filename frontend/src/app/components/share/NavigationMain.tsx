...
const NavigationMain = () => {
  ...
  const handleReadNotifications = async (
    ...
    try {
      ...
      const notiType = data?.message?.type || type;

      // 🔔 Thông báo chủ đề yếu / khóa học yếu
      if (notiType === "weak_topic" || notiType === "weak_course") {
        window.location.href = "/weak-topics";
        return;
      }
                ...
                >
                  {notification && notification.length > 0 ? (
                    notification.map((note) => {
                      const senderName = note.sender?.username || "Hệ thống";
                      const avatarSrc = note.sender?.avatar
                      ...
                      return (
                        <li key={note.id}>
                          ...
                          >
                            ...
                            />
                            <div>
                              <div className="fw-bold">{senderName}</div>
                    ...
                    <p className="px-2 py-1 mb-0">Chưa Có Thông Báo Nào</p>
                  )}
                  <li>
