...
const CoursePending = () => {
  ...
  const GetCourseReject = async () => {
    const res = await fetch("http://localhost:5000/api/teacher_course_reject", {
      method: "GET",
      credentials: "include",
      headers: {
  ...
  const handleDeleteCourse = async(e:React.MouseEvent<HTMLButtonElement>, courseId:string) => {
    e.preventDefault();
                  ...
                  <React.Fragment key={courseItem.id}>
                    <tr>
                      ...
                      <td>
                        ...
                        <button onClick={(e)=>handleDeleteCourse(e,courseItem.id)} className="btn btn-outline-danger">HỦY</button>
                      </td>
                    </tr>
